...
const LandingScreen: React.FC<{ onStartQuiz: () => void }> = ({ onStartQuiz }) => {
  ...
  return (
    ...
    >
      <StickerBadge text="Moon Moon" rotate={-12} className="top-24 left-6 md:left-1/4" variant="white" />
      <StickerBadge text="Dessert" rotate={15} className="top-32 right-6 md:right-1/4" variant="lime" />
      <StickerBadge text="Emotional" rotate={-5} className="bottom-[40%] left-4 z-0 opacity-50 md:opacity-100" variant="lime" />
        ...
        >
          <div className="bg-white/80 backdrop-blur-xl border border-white/60 p-5 md:p-6 rounded-[2.5rem] shadow-[0_20px_40px_rgba(0,0,0,0.15)] text-center ring-1 ring-black/5">
            ...
            <p className="font-sans font-medium text-brand-black mb-6 text-sm leading-relaxed">
              We aren't a mass-market bakery. <br />
              Find your <span className="font-bold underline decoration-brand-lime decoration-4 underline-offset-2">destined sticker</span> in 30 seconds.
            </p>

            <div className="flex flex-col gap-3">
              <Button onClick={onStartQuiz} variant="black" size="lg" fullWidth className="rounded-full shadow-lg text-base h-14">
                Start Quiz
...
const QuizScreen: React.FC<{
  ...
  const handleOptionSelect = (option: Option) => {
    ...
    if (currentQIndex < questions.length - 1) {
      ...
      setIsExiting(true);
      setTimeout(() => onComplete(newAnswers, newSelectedOptions), 400);
    }
  };
  ...
  return (
    <div className={`min-h-screen bg-brand-bg flex flex-col pt-24 px-6 pb-10 transition-all duration-300 ${isExiting ? 'opacity-0 translate-y-4' : 'opacity-100'}`}>
      ...
      <div className="w-full max-w-lg mx-auto mb-12">
        ...
        <div className="h-4 w-full border border-brand-black rounded-full p-0.5 bg-white">
          <div
            className="h-full bg-brand-lime rounded-full transition-all duration-300 border border-brand-black"
